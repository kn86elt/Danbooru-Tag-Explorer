...
- **LLM連携(β)**: 対応するローカルLLMが稼働している場合のみ、日本語の自然言語テキストを Danbooru 風のタグに変換（Ollama / LM Studio 等）。なお1行目の先頭に`/` を入力して変換した場合はタグ変換ではなく通常のLLMとして返答します。
...
- テキスト入力の先頭行1文字目に `/` を付けると、Danbooru タグ変換を行わず LLM への自由質問として送信される
