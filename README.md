...
- **LLM連携(β)**: 対応するローカルLLMが稼働している場合のみ、日本語の自然言語テキストを Danbooru 風のタグに変換（Ollama / LM Studio 等）。1行目の先頭に `/` を入力するとスキルを呼び出せます（タグ分類・シチュエーション生成・ファッションプロンプト生成・Danbooruタグ選択など）。
...
ローカル LLM サーバー（Ollama、LM Studio 等）と連携して、日本語テキストを Danbooru タグに変換できます。ローカルでLLMが稼働していない場合は本機能は動作しません。
...
gemma4 E4B等を想定しています。特に画像生成AIと併用する場合はVRAM使用量が競合します。お使いの環境のVRAM容量に応じて選択してください。
...
- テキスト入力の先頭行に `/` を入力するとスキル候補がドロップダウン表示される
- ↑↓ キーまたは追加入力で絞り込み、Tab / Enter で選択するとスキルの定義テキストが展開される
- 展開後のテキストは自由に編集してから変換できる
- `[temp:N]` をテキスト内に記述すると temperature を上書き指定できる（例: `[temp:0.7]`）
- 先頭行が `/` のみ（スキル名なし）の場合は従来のフリーモード（LLM への自由質問）として動作
...
| `motif` | 女の子をモチーフにイラストシチュエーションとタグを生成 |
| `fashion` | 夏向けファッションの SD プロンプトを複数パターン生成 |
| `danbooru` | LLM のツール呼び出しで danbooru.csv を検索しタグを選択（function calling 対応モデルが必要） |
