...
Stable Diffusion WebUI（A1111 / reForge）の拡張として動作する Danbooru タグブラウザです。
カテゴリツリーや全文検索でタグを探し、スクラッチパッドに蓄積してプロンプト作成に活用できます。
**WebUI のポートをそのまま使うため、追加のポート占有が不要です。**

![スクリーンショット](screenshot.jpg)

## 機能

- **カテゴリツリー探索**: タグを階層ツリーからブラウズ
- **全文インクリメンタル検索**: タグ名・日本語名で AND 検索、最小 POST 数フィルター（非表示件数を表示）
- **タグ詳細モーダル**: Danbooru Wiki 全文・パンくず・関連タグ・`[[tag]]` リンク連鎖表示
- **スクラッチパッド**: タグを蓄積してまとめてコピー（コンマ自動付加）
- **お気に入り・ピン止め**: サーバー側に保存されるため複数端末で共有可
- **LLM 連携（任意）**: 日本語→タグ変換、日本語検索の自動翻訳（Ollama / LM Studio 等）
- モバイル・スマートフォン対応
...
## スタンドアロンで使う（ComfyUI 等 / 動作確認）

A1111 以外の環境や動作確認には、スタンドアロンサーバーとして起動できます。Send To / Read From などの A1111 連携機能を除き、タグ探索・LLM 連携などは同様に使用できます。
...
**日本語検索の自動翻訳:**
日本語で検索して結果がゼロ件の場合、LLM が自動的にタグへ変換して再検索します。検索ボックス右の **🤖** ボタンで手動発火も可能です。翻訳後はクエリのバリアントがチップとして表示され、クリックで切り替えられます。
